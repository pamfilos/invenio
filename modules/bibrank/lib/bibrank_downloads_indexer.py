...
def get_download_weight_total(dic, keys):
    ...
    for k in keys:
        values = run_sql("select count(*) from rnkDOWNLOADS where id_bibrec=%s" , (k,))
        dic[k] = values[0][0]
    return dic
    ...
    for k in keys:
        ...
        for bibdoc in id_bibdocs:
            ...
            for val in values:
                ...
                duplicates = (run_sql("select count(*) from rnkDOWNLOADS where id_bibrec=%s and id_bibdoc=%s and download_time>=%s and download_time<%s and client_host=%s;" , (k, bibdoc[0], date1, date2, val[1]))[0][0])-1 # kwalitee: disable=sql
                run_sql("delete from rnkDOWNLOADS where id_bibrec=%s and id_bibdoc=%s and download_time>=%s and download_time<%s and client_host=%s limit %s;" , (k, bibdoc[0], date1, date2, val[1], duplicates))    # kwalitee: disable=sql
...
def filter_downloads_per_hour (keys, last_updated):
    ...
    for k in keys:
        ...
        for val in values:
            ...
            date1 = "%s:00:00" % (date_res)
            date2 = compute_next_hour(date_res)
            duplicates = (run_sql("""select count(*) from rnkDOWNLOADS where id_bibrec=%s and download_time>=%s and download_time<%s and client_host=%s;""" , (k, date1, date2, val[1]))[0][0])-1   # kwalitee: disable=sql
            run_sql("""delete from rnkDOWNLOADS where id_bibrec=%s and download_time>=%s and download_time<%s and client_host=%s limit %s;""" , (k, date1, date2, val[1], duplicates))  # kwalitee: disable=sql
...
def get_file_similarity_by_times_downloaded(dic, id_bibrec_list):
    ...
    if id_bibrec_list != []:
        ...
        dic_news = {}
        res = run_sql("select id_bibrec,client_host from rnkDOWNLOADS where id_bibrec in %s;" % tuple_string_id_bibrec_list) # kwalitee: disable=sql
        for res_elem in res:
        ...
        for j in dic_news.keys():
            ...
            res2 = run_sql("select id_bibrec,count(*) from rnkDOWNLOADS where client_host in %s and id_bibrec in %s and id_bibrec != %s group by id_bibrec;" %  (tuple_client_host, tuple_string_id_bibrec_list, j)) #0.0023 par requete # kwalitee: disable=sql
